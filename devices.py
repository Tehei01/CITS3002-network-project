...
class Host:
    def __init__(self, name, ip, mac, routing_table, ARP_table):
        ...
        self.links = (
            {}
        )  # Links to other devices: {interface_name: (connected_device, connected_interface)}

        # -------------------------- transport layer state for rdt2.2 ----------------------------
        
        self.sender_expected_seq = 0  # for rdt2.2 (for ACKs) 
    ...
    def l2_send_frame(self, packet, next_hop_ip, out_interface=None):
        ...
        if not next_hop_mac:
            ...
            return
        print(
            f"{self.name}: Layer 2: Destination MAC lookup for next-hop IP ({next_hop_ip}) → {next_hop_mac}"
        )
        frame = Frame(dest_mac=next_hop_mac, src_mac=self.mac, payload=packet, type=ETHERTYPE_IPV4)
        print(
            f"{self.name}: Layer 2: Frame created: SRC_MAC={self.mac}, DST_MAC={next_hop_mac}"
        )
    ...
    def l3_send_packet(self, segment, dst_ip):
        ...
        TTL = DEFAULT_TTL
        print(
            f"{self.name}: Layer 3: Segment received from Transport Layer: SRC_IP={self.ip}, DST_IP={dst_ip}, TTL={TTL}"
        )
        ...
        if not next_hop_ip:
            print(
                f"{self.name}: Layer 3: No route to destination {dst_ip} — packet dropped"
            )
            return
    ...
    def l3_receive_packet(self, packet):
        print(
            f"{self.name}: Layer 3: Packet received from Data Link Layer: SRC_IP={packet.src_ip}, DST_IP={packet.dst_ip}, TTL={packet.ttl}"
        )
        # drop packet if TTL expired
        if packet.ttl <= 0:
            print(f"{self.name}: Layer 3: Packet dropped due to TTL expiry (TTL=0)")
            return
        ...
        if packet.dst_ip == self.ip:
            ...
            print(
                f"{self.name}: Layer 3: Packet identified as not for this host — packet dropped"
            )
    ...
    def l4_send_segment(self, data, data_size, dst_ip, src_port, dst_port, seq_num):
        ...
        print(
            f"{self.name}: Layer 4: Segment created by adding transport layer header (DATA, seq={seq_num}) (encapsulation)"
        )
    ...
    def l4_receive_segment(self, segment, src_ip=None):
        ...
        if not checksum_verify:
            print(
                f"{self.name}: Layer 4: Checksum verification failed — segment dropped"
            )
            if self.last_sent_ack is not None and self.last_sent_ack_ip is not None:
                ...
                print(
                    f"{self.name}: Layer 4: Segment created by adding transport layer header (ACK, seq={self.last_sent_ack.seq_num})"
                )
                print(f"{self.name}: Layer 4: Segment sent to Network Layer")
        ...
        if segment.seg_type == segment.DATA:
            ...
            if segment.seq_num == self.receiver_expected_seq:
                print(
                    f"{self.name}: Layer 4: DATA segment delivered to Application Layer. Data size={len(segment.data)}"
                )
                ack_seq = segment.seq_num
                self.receiver_expected_seq = 1 - self.receiver_expected_seq
                ack_segment = Segment(
                    # switch source and destination for ACK
                ...
                print(
                    f"{self.name}: Layer 4: Segment created by adding transport layer header (ACK, seq={ack_seq})"
                )
                print(f"{self.name}: Layer 4: Segment sent to Network Layer")
                self.l3_send_packet(ack_segment, src_ip)

            elif segment.seq_num != self.receiver_expected_seq:
                # resend last ACK to trigger retransmission in rdt2.2
                if self.last_sent_ack is not None and self.last_sent_ack_ip is not None:
                    print(
                        f"{self.name}: Layer 4: Duplicate DATA segment received with sequence number {segment.seq_num} (expected {self.receiver_expected_seq})"
                    )
                    print(
                        f"{self.name}: Layer 4: Segment created by adding transport layer header (ACK, seq={self.last_sent_ack.seq_num})"
                    )
        ...
        # --------- unexpected ACK handling for rdt2.2 ---------
            ...
            if segment.seq_num == self.sender_expected_seq:
                ...
                """
                print(
                    f"{self.name}: Layer 4: ACK received with unexpected sequence number: seq={segment.seq_num} (expected {self.sender_expected_seq})"
                )
                print(
                    f"{self.name}: Layer 4: Retransmitting segment with seq={self.sender_expected_seq}"
                )
...
class Router:
    def __init__(self, name, Interfaces, routing_table, ARP_table):
        self.name = name
        self.Interfaces = (
            Interfaces  # List of (interface_name, IP, MAC) tuples for each interface
        )
    ...
    def l2_send_frame(self, packet, next_hop_ip, out_interface=None):
        ...
        frame = Frame(dest_mac=next_hop_mac, src_mac=interface_mac, payload=packet, type=ETHERTYPE_IPV4)
        print(
            f"{self.name}: Layer 2: Frame created: SRC_MAC={interface_mac}, DST_MAC={next_hop_mac}"
        )
    ...
    def l2_receive_frame(self, frame, in_interface=None):
        ...
        print(
            f"{self.name}: Layer 2: Source MAC learned: {frame.src_mac} on {in_interface}"
        )
        print(f"{self.name}: Layer 2: Packet delivered to Network Layer")
        print()
    ...
    def l3_forward_packet(self, packet):
        ...
        if not next_hop_ip:
            print(
                f"{self.name}: Layer 3: No route to destination {packet.dst_ip} — packet dropped"
            )
            return
        print(f"{self.name}: Layer 3: Next-hop IP determined: {next_hop_ip}")
...
# ------------------------- Helper functions --------------------------------
def lookup_route(dst_ip, routing_table):  # test
    ...
    return (None, None)  # no matching route
